...
class NemotronHModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Eagle3 aux hidden state layer capture support. Mirrors the
        # qwen2 / llama / etc. pattern -- captures the residual stream
        # BEFORE each layer in `layers_to_capture`. SGLang's convention
        # uses "input to layer k", so a SpecForge-trained config of
        # `eagle_aux_hidden_state_layer_ids = [2, 26, 48]` (which means
        # "output of layers 2/26/48") becomes `[3, 27, 49]` here -- the
        # +1 conversion is done in `set_eagle3_layers_to_capture` on the
        # ForCausalLM class.
...
class NemotronHForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Eagle3 capture flag, set by set_eagle3_layers_to_capture below.
    ...
    def forward(
        ...
        # Eagle3 aux hidden state propagation. When spec decode is active,
        # NemotronHModel.forward returns a tuple
        # (hidden_states, aux_hidden_states). Unpack and pass aux through
        # to the logits processor like qwen2 / llama do.
    ...
    def set_eagle3_layers_to_capture(self, layer_ids: Optional[list[int]] = None):
        # Eagle3 layer capture installer, mirroring the qwen2
        # implementation. Convention: SpecForge stores the trained
        # `eagle_aux_hidden_state_layer_ids` as "output of layer k", but
        # SGLang's capture loop fires BEFORE running each layer, so a
        # value of `k` here means "input to layer k = output of layer k-1".
        # The +1 conversion below makes them agree.
        if not self.pp_group.is_last_rank:
            return
        self.capture_aux_hidden_states = True
        if layer_ids is None:
            num_layers = len(self.config.hybrid_override_pattern)
